...
const ACTION_TIME_COST: Record<string, number> = {
  move:    0.25,
  speak:   0.25,
  take:    0.1,
  examine: 0.1,
...
// Applique les dégradations passives des stats de l'entité contrôlée
export function applyPassiveDecay(entity: Entity, hours: number): Entity {
  if (entity.hunger === undefined && entity.fatigue === undefined) return entity;
  return {
    ...entity,
    hunger:  entity.hunger  !== undefined ? Math.max(0, entity.hunger  - HUNGER_DECAY_PER_HOUR  * hours) : undefined,
    fatigue: entity.fatigue !== undefined ? Math.max(0, entity.fatigue - FATIGUE_DECAY_PER_HOUR * hours) : undefined,
  };
}

// Retourne le coût en heures d'un type d'action
export function getTimeCost(actionType: string): number {
  return ACTION_TIME_COST[actionType] ?? 0;
}
